...
router.get("/fetchjobs", fetchuser, cache("jobs", 300), async (req, res) => {
  try {
    const job = await Job.find();
    return res.json(job);
  } catch (error) {
...
router.post(
  ...
  isAdmin,
  [
    body("type", 'Type must be either "job" or "internship"').isIn([
      "job",
      "internship",
    ]),
    body("companyName", "Company name is required").notEmpty(),
    body("closingDate", "Closing date is required and must be a valid date")
      .isISO8601()
      .toDate(),
    body("requiredGPA", "GPA must be a number between 0 and 10").isFloat({
      min: 1,
      max: 10,
    }),
    body("batch", "Batch must be from 2024 to 2027").isInt({
      min: 2024,
      max: 2027,
    }),
  ],
  async (req, res) => {
    try {
      ...
      console.log("🧹 Cache invalidated: jobs");
