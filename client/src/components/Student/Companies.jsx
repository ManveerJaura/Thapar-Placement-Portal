...
const Companies = () => {
  ...
  // ✅ Add another effect that runs when data is fetched
  useEffect(() => {
    if (companies.length > 0 && student && student.gpa && student.year) {
      const filtered = companies.filter(company => {
        return (
          company.requiredGPA <= parseFloat(student.gpa) &&
          company.batch == parseInt(student.year) &&
          new Date(company.closingDate) >= Date.now()
        );
      });
      setfilterComp(filtered);
    }
  }, [companies, student]); // 🔥 runs whenever data updates
